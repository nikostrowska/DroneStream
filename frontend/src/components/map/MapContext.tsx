...
export default function MapContext({ telemetry, pilot }: { telemetry: DroneTelemetry | undefined, pilot: DroneTelemetry | undefined }) {
  ...
  const dronePointRef = useRef<Point | undefined>(undefined);
  const droneFeatureRef = useRef<Feature | undefined>(dronePoint.current);
  ...
  const pilotPoint = useRef<Point | undefined>(undefined);
  const pilotFeature = useRef<Feature | undefined>(pilotPoint.current);
  const pilotStyle = useMemo(() => {
    new Style({
      image: new Icon({
        anchor: [0.5, 1],
        src: markerBlue,
